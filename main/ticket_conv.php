<?php 
    ...
    $status = $pdo->getStatusTicket($_GET['id']);

    if($status == "Fixed"){
        $input_send = "disabled";
        $input_message = "disabled";
        $message_placeholder = "This ticket is closed. You can't answer";
    } else {
        $input_send = "";
        $input_message = "";  
        $message_placeholder = "Type a message and click send to send message...";
    }
        ...
        <div class="content__inner">              
            <div class="messages">
                <div class="messages__body">
                    ...
                    <?php if($status == "Fixed"){ ?>
                        <div style="margin: 0;" class="alert alert-success">THIS TICKET IS CLOSED</div>
                    <?php } ?>
                    ...
                    <div class="messages__reply">                        
                        <form action="../action/add/send_ticket_message.php" method="POST">
                            <textarea id="chat_msg" name="message" class="messages__reply__text" placeholder="<?php echo $message_placeholder ?>" style="min-height: 200px;" <?php echo $input_message ?>></textarea>

                            <input type="hidden" name="id_ticket" value="<?php echo $_GET['id']?>">
                            <input type="submit" class="btn btn-success btn-block" value="SEND" <?php echo $input_send ?>>
                            
                            <?php if(strtoupper($_SESSION['type']) == "ADMIN") { ?>
                                <a href="../action/delete/close_ticket.php?ticket=<?php echo $_GET['id']?>" class="btn btn-warning" id="close_ticket">Close this ticket</a>
                            <?php } ?>
                        </form>
                    </div>
                </div>
